-- Task #4375 security follow-up: the atomic mapping RPC is service-only, but
-- must still enforce that a preference field belongs to this tenant and is an
-- organization field before writing an organization preference value.
...
BEGIN
  ...
  PERFORM 1 FROM public.form_submission_due_diligence
   WHERE id = p_due_diligence_submission_id AND tenant_id = p_tenant_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'due-diligence submission is not tenant-owned' USING ERRCODE = 'P0001';
  END IF;

  SELECT * INTO v_organization FROM public.organization
  ...
  IF p_event_type = 'core' THEN
    IF EXISTS (
      SELECT 1 FROM jsonb_object_keys(COALESCE(p_mutation, '{}'::JSONB)) AS key_name
    ...
    INSERT INTO public.form_due_diligence_field_mapping_workflow_outbox (
      ...
      p_due_diligence_submission_id, p_tenant_id, p_event_key, 'core', p_organization_id,
      jsonb_build_object('before', v_before, 'after', v_after)
    ) ON CONFLICT (form_submission_due_diligence_id, event_key) DO NOTHING;
    RETURN jsonb_build_object('applied', TRUE, 'created', FALSE, 'before', v_before, 'after', v_after);
  END IF;

  IF p_preference_field_id IS NULL THEN
    RAISE EXCEPTION 'preference field is required' USING ERRCODE = '22023';
  END IF;
  -- `organization` is the established preference_field entity_scope value
  -- (organization_group is a separate allowed scope and is rejected here).
  PERFORM 1 FROM public.preference_field
  ...
  SELECT value INTO v_preference_value FROM public.organization_preference_value
   WHERE organization_id = p_organization_id AND field_id = p_preference_field_id FOR UPDATE;
  ...
  IF v_preference_exists THEN
    UPDATE public.organization_preference_value SET value = p_preference_value, updated_at = NOW()
  ...
  INSERT INTO public.form_due_diligence_field_mapping_workflow_outbox (
    ...
    p_due_diligence_submission_id, p_tenant_id, p_event_key, 'preference', p_organization_id,
    jsonb_build_object(
      'field_id', p_preference_field_id, 'previous_value', v_preference_value,
