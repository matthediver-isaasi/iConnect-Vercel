-- Task #1760: Count-based ticket availability for COMPLEX (multi-session) events.
--
-- Mirrors the standard-event guard (check_oneoff_ticket_capacity, Task #1758)
-- for complex events. complex_event_ticket_class.available_count is treated as a
-- fixed admin-set MAXIMUM (capacity) and is never mutated by bookings.
-- Availability is derived from the actual count of status='confirmed' rows in
-- complex_event_booking per (event, ticket class).
--
...
-- (complex event, ticket class) are serialized with a transaction-scoped
-- advisory lock (namespaced 'cx:' so it never collides with the one-off guard).
--
-- Two modes:
...
-- complex_event_booking.ticket_class_id is stored as text; the ticket class id
-- is a uuid, so the class lookup compares id::text = p_ticket_class_id.
-- Idempotent: CREATE OR REPLACE.

create or replace function check_complex_event_ticket_capacity(
  p_event_id uuid,
...
declare
  v_lock_key bigint;
  v_tc complex_event_ticket_class%rowtype;
  v_max integer;
  v_sold integer;
  v_rank integer;
  v_our_count integer;
...
begin
  ...
  if p_event_id is null or p_ticket_class_id is null or btrim(p_ticket_class_id) = '' then
  ...
  v_lock_key := hashtextextended('cx:' || p_event_id::text || ':' || p_ticket_class_id, 0);
  perform pg_advisory_xact_lock(v_lock_key);

  -- Resolve the admin-set maximum (capacity) from the ticket class row.
  select * into v_tc
  from complex_event_ticket_class
  where id::text = p_ticket_class_id
    and complex_event_id = p_event_id
  limit 1;

  -- No matching ticket class, unlimited flag, or no maximum => no cap.
  if not found
     or coalesce(v_tc.is_unlimited_tickets, false)
     or v_tc.available_count is null
  then
    return jsonb_build_object('ok', true, 'unlimited', true);
  end if;

  v_max := v_tc.available_count;
  ...
  delete from complex_event_booking where id = any(p_booking_ids);

  return jsonb_build_object('ok', false, 'sold', v_rank, 'max', v_max, 'unlimited', false);
end;
$$;

-- Live availability for complex events is derived from confirmed
-- complex_event_booking rows, so the realtime hook subscribes to that table.
-- Add it to the supabase_realtime publication (idempotent) so those changes are
-- actually broadcast — mirrors the standard-event `booking` table.
do $$
begin
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and tablename = 'complex_event_booking'
  ) then
    alter publication supabase_realtime add table complex_event_booking;
  end if;
end;
$$;
