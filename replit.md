...
-   **Communication:** Includes an email template system, communication preferences, and email campaigns with a list-based targeting model. Lists (`audience_list` table) are standalone, category-independent collections of audience segment rules (role, member_group, all_members, form, fundraisers, donors, communication_category). Campaigns select one or more Lists plus a Subscription Category (`communication_category_id` on `email_campaign`). At send time, a two-step filtering funnel is applied: (1) remove members with global opt-out (`communications_opted_out_all` or `email_unsubscribe` type `all`), (2) remove members not opted in to the campaign's selected category via `member_communication_preference`. List management lives in CommunicationsManagement.jsx as an independent "Lists" section. Team invitations support per-role invite email templates and sign-up links (stored on the `role` table as `invite_email_template_id` and `signup_link_template`), with fallback to global tenant-level settings in `system_settings`.
