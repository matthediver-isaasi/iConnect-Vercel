...
-   **Field Visibility Rules:** Conditional show/hide rules for fields and cards on both organisation and member detail views. Rules use AND/OR logic with conditions (equals, contains, is_empty, etc.) and actions (show/hide field or card). Stored in `SystemSettings` as `org_field_visibility_rules` and `member_field_visibility_rules`. Shared evaluation logic in `useOrgFieldVisibilityRules.js` (`evaluateVisibilityRules()`, `OPERATORS`). Organisation editor: `OrgFieldVisibilityRulesEditor.jsx`, integrated in `OrganisationDetailView.jsx`. Member editor: `MemberFieldVisibilityRulesEditor.jsx`, integrated in `MemberDetail.jsx`. Member hook: `useMemberFieldVisibilityRules.js`.
...
-   **Dynamic Directory:** Configurable member/organisation directories with slug-based URLs. Member directories use server-side filtering and pagination via `api/dynamic-directory/members.js` (filters by `filter_field_id`/`filter_value` from directory config, custom field filters, search, sort, pagination) and `api/dynamic-directory/member-preferences.js` (fetches preference values for displayed members only). Organisation dropdown is hidden for member-type directories (`entity_type === 'member'`). Frontend: `DynamicDirectoryView.jsx`, management: `DynamicDirectoryManagement.jsx`.
...
-   **Member Membership API:** `api/membership/member-membership.js` provides combined membership data (config, year costs, history, override metadata) for the admin member detail view. When a year is recorded (history exists), the API builds year data purely from the history record — no simulation is run. All breakdown fields (VAT, pro-rata days, free period days, year number, override info) are stored at recording time in the history tables. For old records missing the new columns, org API falls back to recalculating pro-rata/free period days from config+go-live date.
-   **Membership Tab UX:** Override button is hidden on year cards when the year has been paid/invoiced (history record exists). VAT breakdown (rate, amount, total incl. VAT) is shown in year cost cards and simulation result dialogs in both `MemberMembershipTab.jsx` and `OrgMembershipTab.jsx` when applicable.
