...
-   **Import BNMS posters CSV (batch 2/3, one-off, idempotent):** `node scripts/import-bnms-resources2.mjs [--dry-run] [--create-missing-categories] [--csv=<path>] [--tenant=<uuid>] [--limit=N]`. Loads the semicolon-delimited BNMS posters CSV into `resource` for tenant BNMS (`ff2df806-b321-4254-b651-3af11fccf1db`). Differs from the Task #1087 script: `;` delimiter, focus-area cells marked "X" (not "Yes"), all rows forced `is_public=true`, ignores the extra "Collection" column. `--create-missing-categories` adds "Posters" to "Resource Type" and "Artificial intelligence" to "Focus Area". Matches existing rows on (`tenant_id`,`target_url`) then `title`; writes run with bounded concurrency. The batch-2 and batch-3 CSVs (`bnms-resources2_*.csv`, `bnms-resources3_*.csv`) are byte-identical (same 921 URLs), so a single completed run covers both.
