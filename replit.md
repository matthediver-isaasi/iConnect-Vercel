...
-   **GoCardless tests:** `node --test api/_lib/gocardless*.test.mjs` — webhook signature verification, idempotency keys, status-transition matrix, the idempotent/out-of-order-tolerant webhook processor (fake db + injected GC client), per-tenant credential resolution (tenant_integrations → env fallback, env/token mismatch guards), the org DD billing-contact invitation suite (token format, expiry clamp 1-90 via `dd_invite_expiry_days`, validate/supersede/single-use/revoke, org email recipient dedupe), the Phase 5 renewal decision suite (`gocardlessDdRenewals.test.mjs`: next-year label derivation, notice window, auto vs confirmation-required renewal decisions, terminal-row idempotence), and the migration invite/funnel suite (`gocardlessDdMigration.test.mjs`: token single-use/supersede/expiry validation, conversion funnel stage derivation).
