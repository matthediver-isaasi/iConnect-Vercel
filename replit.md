...
-   **Field Visibility Rules:** Implements conditional show/hide rules for fields and cards on organisation and member detail views, based on AND/OR logic. Member custom fields (preference fields) support role-based visibility via `my_preferences_role_ids` (JSONB on `preference_field` table): null/empty = all roles, array of role UUIDs = restricted. Filtering applied in `Preferences.jsx` (member-facing) and `MemberPreferences.jsx` (admin config). Organisation custom fields support the same role-based visibility via `my_organisation_role_ids` (JSONB on `preference_field` table): null/empty = all roles, array of role UUIDs = restricted. Filtering applied in `MyOrganisation.jsx` (member-facing). Admin UI in `CustomFieldsAdmin.jsx` provides a multi-select role picker when My Preferences or My Organisation toggle is on.
