...
-   **GoCardless tests:** `node --test api/_lib/gocardless.test.mjs api/_lib/gocardlessWebhookProcessor.test.mjs api/_lib/gocardlessCredentials.test.mjs api/_lib/gocardlessDdInvitations.test.mjs api/_lib/gocardlessArrears.test.mjs` — webhook signature verification, idempotency keys, status-transition matrix, the idempotent/out-of-order-tolerant webhook processor (fake db + injected GC client), per-tenant credential resolution (tenant_integrations → env fallback, env/token mismatch guards), and the org DD billing-contact invitation suite (token format, expiry clamp 1-90 via `dd_invite_expiry_days`, validate/supersede/single-use/revoke, org email recipient dedupe).
