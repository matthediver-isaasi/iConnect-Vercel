...
-   **AI assistant tests:** `node --test api/_lib/*.test.mjs client/src/components/canvas/autoHeightBake.test.mjs client/src/components/canvas/useAutoHeightBake.test.mjs` — runs the member-AI visibility (security boundary), ranking, indexer, and help-chunker suites, plus the Canvas auto-height bake corruption-guard regression suite (pure decision logic) and the `useAutoHeightBake` hook's runtime-gate suite (jsdom mount: settle gate + breakpoint re-arm, author-intent gate, content-ready re-check). Registered as the `ai-assistant-tests` validation step, so it runs automatically on task completion.
