...
-   **WordPress Sync:** iConnect articles sync to WordPress sites via the iConnect Content Sync plugin. Articles are served from `/api/public/articles` (tenant-scoped). When articles are created, updated, or deleted, a webhook notification is dispatched to the tenant's configured WordPress webhook URL (`wp_webhook_url` and `wp_webhook_api_key` system settings). Settings managed via `api/admin/wp-sync-settings.js` (admin-only). UI in Admin Integrations page (`AdminIntegrations.jsx`). Webhook dispatch helper: `api/_lib/wpWebhook.js` (fire-and-forget, non-blocking). Hooks into `BlogPost` entity create/update/delete in generic entity handlers.
