...
-   **GoCardless tests:** `node --test api/_lib/gocardless.test.mjs api/_lib/gocardlessWebhookProcessor.test.mjs api/_lib/gocardlessCredentials.test.mjs` — webhook signature verification, idempotency keys, status-transition matrix, the idempotent/out-of-order-tolerant webhook processor (fake db + injected GC client), and per-tenant credential resolution (tenant_integrations → env fallback, env/token mismatch guards).
