...
-   **Import GSF verified domains from CSV (gsf only, idempotent):** `node scripts/import-gsf-verified-domains.mjs [--apply] [--verbose] [--csv=<path>] [--org=<uuid>]`. Hard-pinned to tenant `21296ad6-1350-483a-a90c-1b06ece70501` and to the `verified_domains` preference_field; replaces (authoritative) each org's `organization_preference_value` row for `verified_domains` with the deduped+lowercased CSV set. Dry-run by default.
...
-   **Grant Sharon GFI admin access (Task #1164, one-off, idempotent):** `node scripts/fix-gfi-sharon-admin-access.mjs [--apply]`. Hard-pinned to the single GFI `tenant_membership` row `05a31060-…` (identity `dc156a30-…`, tenant `fd82da65-…`); flips `role`/`membership_type` to `owner` and re-reads to verify. Dry-run by default; safe no-op once applied. Pairs with the session/login admin-access hardening that fixed the member-only 401 ("cannot coerce result to single JSON object").
-   **Unlimited plan + existing-tenant backfill (one-off, idempotent):** `node scripts/apply-unlimited-plan-existing-tenants.mjs`. Applies `supabase/migrations/20260529_unlimited_plan_existing_tenants.sql` against `DEST_DATABASE_URL` (pooler). Inserts the non-self-serve `unlimited` plan (all quotas null = no limits, `custom_domain` true, no Stripe price) and moves every existing tenant to `plan_code='unlimited'` so organisations backfilled to `free` aren't silently throttled. Re-runnable with no double effects (`ON CONFLICT (code) DO NOTHING` + idempotent UPDATE). New tenants still default to `free`.
-   **Import BNMS posters CSV (batch 2, one-off, idempotent):** `node scripts/import-bnms-resources2.mjs [--dry-run] [--create-missing-categories] [--csv=<path>] [--tenant=<uuid>] [--limit=N]`. Loads the semicolon-delimited BNMS posters CSV into `resource` for tenant BNMS (`ff2df806-b321-4254-b651-3af11fccf1db`). Differs from the Task #1087 script: `;` delimiter, focus-area cells marked "X" (not "Yes"), all rows forced `is_public=true`, ignores the extra "Collection" column. `--create-missing-categories` adds "Posters" to "Resource Type" and "Artificial intelligence" to "Focus Area". Matches existing rows on (`tenant_id`,`target_url`) then `title`; writes run with bounded concurrency. NOTE: the original batch-3 upload was a byte-identical duplicate of this batch-2 posters file — the *real* batch-3 (YouTube videos) is a separate dataset imported by the script below.
-   **Import BNMS videos CSV (corrected batch 3, one-off, idempotent):** `node scripts/import-bnms-resources3.mjs [--dry-run] [--create-missing-categories] [--csv=<path>] [--tenant=<uuid>] [--limit=N]`. Loads the corrected semicolon-delimited BNMS *videos* CSV (`attached_assets/BNMS-batch3-correct_*.csv`) into `resource` for tenant BNMS (`ff2df806-b321-4254-b651-3af11fccf1db`) as `resource_type='external_link'` + `open_in_new_tab=true`. Maps `Video URL`→`target_url`, `Date uploaded`→`release_date`; derives `is_public` from "Member Only" ("No"→public, "Yes"→member-only). Extended date parser (DD-MMM-YY, D MMM YYYY incl. 4-letter "Sept", d/m/yy 2-digit year, plus the batch-2 formats). De-dupes within the file by extracted YouTube video id (fallback normalized title), normalizes scheme-less `youtube.com/…` URLs to `https://`, and skips+reports rows with no usable URL (one source row had its title pasted into the URL column). Result: 189 video resources (153 public, 36 member-only). Matches existing rows on (`tenant_id`,`target_url`) then `title`.
-   **Import GSF newsletter sign-ups CSV (gsf only, one-off, idempotent):** `node scripts/import-gsf-newsletter-subscribers.mjs [--apply] [--verbose] [--csv=<path>] [--limit=N]`. Hard-pinned to tenant `21296ad6-1350-483a-a90c-1b06ece70501`. Loads the semicolon-delimited Zoho newsletter export into `email_subscriber` as non-member list subscriptions (mirrors the anonymous form-submission path). Writes one row per person per category — Newsletter (`0043b368-…`), Job opportunities (`b33c53c3-…`), Event Updates (`478810ca-…`) — with `opted_out = !tick`; `All opt out=true` forces opted_out on all three. Lowercases email, `form_id=null`, "Job title"/"Free text organisation" not stored (no columns). Diffs against existing rows (preserves `subscribed_at`), upserts on (`tenant_id,email,communication_category_id`). Dry-run by default; ~874 people → 2622 rows.
-   **Boolean preference-value canonicalisation backfill (one-off, idempotent):** `node scripts/backfill-boolean-preference-values.mjs [--apply] [--tenant=<uuid>] [--verbose]`. Scans `organization_preference_value` / `member_preference_value` / `job_posting_preference_value` for rows whose `field_id` references a boolean/checkbox `preference_field` and whose `value` is not already `'true'`/`'false'`, runs each through `coerceBooleanPreferenceValue`, and rewrites to the canonical string. Rows that don't map cleanly (ambiguous text on a mis-typed field) are left alone and reported. Pairs with the workflow `update_field` boolean-coercion fix so legacy workflow-written values render correctly in the Switch toggle. Dry-run by default.
