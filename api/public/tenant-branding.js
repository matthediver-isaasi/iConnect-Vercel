...
export function createTenantBrandingHandler({
  db = supabase,
  resolveTenant = resolveTenantFromRequest,
  resolveMicrosite = resolveMicrositeByPrefix,
  resolveBranding = resolveTenantBrandingPayload,
  ...
  try {
    const tenantData = await resolveTenant(req);

    if (!tenantData) {
      console.log('[Tenant Branding] Tenant not found');
      return res.status(404).json({ error: 'Tenant not found' });
    }
    const viewer = await resolveViewer(req, tenantData.id);
    ...
    if (micrositePrefix) {
      microsite = await resolveMicrosite(db, tenantData.id, micrositePrefix);
      if (!microsite) {
    ...
    res.json({
      ...
      branding: await resolveBranding(
        tenantData,
        microsite,
        { allowMemberOnlyContent: viewer.allowMemberOnlyContent }
      ),
    });
  } catch (error) {
...
export default createTenantBrandingHandler();
