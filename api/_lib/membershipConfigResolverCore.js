export function createMembershipConfigResolver(supabase) {
return { resolveRollingSuccessorConfig, findHistoricalMemberConfigs, getAllActiveConfigs, getAllActiveConfigsStrict, getConfigForOrganisation, resolveBasisFieldLabel, resolveBasisFieldLabels, getConfigByIdDirect, getConfigForMember };
...
async function resolveRollingSuccessorConfig(client, { tenantId, previousTerm }) {
 ...
 */
function findHistoricalMemberConfigs(historyRecords = []) {
...
async function getAllActiveConfigs(tenantId, onDate = null) {
...
// above.
async function getAllActiveConfigsStrict(tenantId, onDate = null) {
  const asOf = onDate || new Date().toISOString().split('T')[0];
  const { data, error } = await supabase
    .from('membership_tier_config')
    .select('*')
...
async function resolveBasisFieldLabel(config, tenantId) {
  if (!config) return 'Value';
...
async function resolveBasisFieldLabels(configs, tenantId) {
...
async function getConfigByIdDirect(tenantId, configId) {
  const { data } = await supabase
    .from('membership_tier_config')
    .select('*')
    .eq('id', configId)
...
async function getConfigForMember(tenantId, memberId, fieldOverrides = {}, onDate = null) {
  const allConfigs = await getAllActiveConfigs(tenantId, onDate);
  if (!allConfigs || allConfigs.length === 0) return null;
