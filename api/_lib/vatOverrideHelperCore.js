...
export function createVatOverrideHelper(supabase) {
return { evaluateVatOverrideForOrg, evaluateVatOverrideForMember };

async function evaluateVatOverrideForOrg(configId, tenantId, organizationId, fieldOverrides = {}) {
  try {
...
async function evaluateVatOverrideForMember(configId, tenantId, memberId, fieldOverrides = {}) {
  try {
    const { data: overrideRules, error: overrideError } = await supabase
      .from('membership_tier_vat_override')
      .select('*')
