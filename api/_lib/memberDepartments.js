...
export async function resolveMemberDepartmentDefinition(db, tenantId, { required = false } = {}) {
  ...
  if ((definitions || []).length !== 1
    ...
    || definitions[0].cardinality !== 'one_to_many') {
...
export async function resolveDepartmentMemberIds(db, tenantId, departmentIds) {
  ...
  const { definitionId } = await resolveMemberDepartmentDefinition(db, tenantId, { required: true });
  return [...new Set((await pageRows((from, to) => db.from('custom_object_relationship')
    .select('target_record_id').eq('tenant_id', tenantId).eq('relationship_definition_id', definitionId)
    .is('archived_at', null).in('source_record_id', ids).range(from, to))).map(row => row.target_record_id))];
}
...
export async function enrichMembersWithDepartments(db, tenantId, members) {
  ...
  if (!schema) return members.map(row => ({ ...row, department: null, department_id: null }));
  ...
  if (!departmentIds.length) return members.map(row => ({ ...row, department: null, department_id: null }));
  ...
  const byMember = new Map(edges.map(edge => [edge.target_record_id, edge.source_record_id]));
  return members.map(row => {
    const id = byMember.get(row.id);
    const department = id ? { id, name: names.get(id) || '', organization_id: row.organization_id || null } : null;
    return { ...row, department, department_id: id || null, ...(department ? { department_organization_name: orgNames.get(row.organization_id) || '' } : {}) };
  });
}
...
export async function listDepartmentOptions(db, tenantId, organizationIds = []) {
  ...
  const { data: parents, error } = await db.from('custom_object_relationship_definition').select('id')
  ...
  if (error || (parents || []).length !== 1) throw new MemberDepartmentError('Department organisation schema is unavailable', 409);
  ...
  const relevant = parentEdges.filter(edge => !requested.size || requested.has(edge.target_record_id));
  ...
  return relevant.map(e => ({ id: e.source_record_id, name: recordMap.get(e.source_record_id)?.data?.[field.data?.name] || '', organization_id: e.target_record_id, organization_name: orgMap.get(e.target_record_id) || '' }))
    .filter(row => recordMap.has(row.id)).sort((a, b) => a.name.localeCompare(b.name));
}