...
export async function loadPaymentReportDirectDebit(input, { db,
  ...
  async function enrich(plan) {
    ...
    const pending = payments.filter(p => p.tenant_id === tenantId && p.plan_id === plan.id
      ...
      && ['pending_submission', 'submitted'].includes(p.status) && p.charge_date >= today)
    ...
    if (next) {
      ...
      if (detail.nextPaymentAmount !== null) detail.nextPaymentAmountState = 'Provider-scheduled payment';
    }
    if (plan.metadata?.collection_mode === 'dynamic') {
      ...
      try {
        ...
        if (detail.nextPaymentAmount === null) detail.nextPaymentAmountState = 'Review required — amount or currency invalid';
      } catch {
