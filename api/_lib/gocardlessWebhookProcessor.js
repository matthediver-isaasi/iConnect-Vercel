  ...
  if (action === 'fulfilled') {
    ...
    try {
      ...
      const { data: form } = await db
        .from('form')
        .select('id, name, tenant_id, fields, pages, visibility_rules, entity_pipelines, structured_actions, field_mappings, application_level, create_entity_type, entity_action, member_entity_action, organization_entity_action, additional_member_creations, submission_emails, submission_email_template_id, submission_email_recipient, submission_email_cc, submission_email_bcc, submission_email_field_mapping, form_type, due_diligence_required, survey_settings')
        .eq('id', row.form_id)
