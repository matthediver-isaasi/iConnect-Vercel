...
// How long to wait for each worker dispatch before abandoning the caller's
// side of the connection. The downstream worker invocation runs to
// completion independent of this signal.
...
async function dispatchWorker(origin, job) {
  const url = `${origin}/api/admin/form-submission-export-jobs/process?jobId=${encodeURIComponent(job.id)}&token=${encodeURIComponent(job.worker_token)}`;
  ...
  try {
    ...
    if (err && err.name === 'AbortError') {
      // Expected: we aborted the caller wait after the request was sent.
      return { jobId: job.id, ok: true };
    }
    console.warn('[cron/run-form-submission-export-jobs] trigger failed:', err?.message);
...
export default async function handler(req, res) {
  ...
  if (queuedErr) {
    console.error('[cron/run-form-submission-export-jobs] queued query error:', queuedErr);
  }
  ...
  if (remaining > 0) {
    const { data: staleRows, error: staleErr } = await supabase
      ...
      .limit(remaining);
    if (staleErr) {
      console.error('[cron/run-form-submission-export-jobs] stale query error:', staleErr);
    }
    stale = staleRows || [];
  }
  ...
  // Deterministic dispatch: await every trigger (with bounded abort) so the
  // cron tick returns a truthful record of which jobs were actually kicked.
  ...
  if (failed.length) {
    console.warn('[cron/run-form-submission-export-jobs] dispatch failures:', failed);
  }
