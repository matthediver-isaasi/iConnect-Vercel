...
export default async function handler(req, res) {
  ...
  try {
    ...
    const query = 'SELECT Id, Name, Active FROM TaxCode WHERE Active = true MAXRESULTS 500';
    ...
    if (!resp.ok) {
      ...
      return res.status(502).json({ error: `QBO TaxCode query failed: HTTP ${resp.status} ${text.slice(0, 300)}` });
    }
    ...
    const msg = err?.message || 'Failed to fetch QuickBooks tax codes';
