...
async function verifyAdminAccess(req) {
  ...
  try {
    ...
    console.error('[Admin Update Badge Access Verify] Error:', error);
...
export default async function handler(req, res) {
  ...
  const { bookingId, badge } = req.body || {};
  ...
  const valueToStore = badge === true || badge === 'true';
  ...
  try {
    ...
    if (updateError) {
      console.error('[Admin Update Badge] Update failed:', updateError);
      return res.status(500).json({ error: updateError.message || 'Failed to update badge' });
    }

    console.log(
      `[Admin Update Badge] Booking ${bookingId} (${table}, event ${eventId}) badge set to ${valueToStore}`
    );

    return res.status(200).json({
      success: true,
      bookingId: updated.id,
      badge: updated.badge !== false,
    });
  } catch (error) {
    console.error('[Admin Update Badge] Error:', error);
    return res.status(500).json({ error: error.message || 'Failed to update badge' });
  }
}
