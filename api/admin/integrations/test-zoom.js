  ...
  try {
    ...
    try {
      ...
      if (userResponse.ok) {
        ...
        if (firstUser) {
          account_email = firstUser.email;
          account_type = firstUser.type === 1 ? 'Basic' : firstUser.type === 2 ? 'Licensed' : 'Admin';
        }
      }
    } catch (e) {
      console.log('[Zoom Test] Could not fetch user info (non-critical):', e.message);
    }
    ...
    res.json({ 
      ...
      account_email: account_email || 'N/A',
