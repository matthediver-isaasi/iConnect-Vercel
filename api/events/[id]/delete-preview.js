// Admin preview endpoint for the safe-delete flow (task-700).
// Returns counts and an estimated financial impact summary so the admin can
// see exactly what is about to happen before they confirm DELETE EVENT.
// Same shape served by api/complex-events/[id]/delete-preview.js.
...
export default async function handler(req, res) {
  ...
  const authz = await authorizeGroupAdminEventDelete({ eventId: id, eventTable: 'event', tenantCtx: ctx, req });
  if (!authz.ok) return res.status(authz.status || 403).json({ error: authz.error });

  const result = await previewEventDeletion({ eventId: id, tenantId: ctx.tenantId, eventTable: 'event' });
