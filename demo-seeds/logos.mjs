...
export async function linkPrimaryOrgLogo({ sb, tenantId, log = console.log }) {
  ...
  if (!brandingLogo) {
    log('[demo-logo] warning: tenant has no branding logo yet — skipping primary org logo link (see demo-seeds/README.md, "Images (avatars, logos & event headers)")');
    return false;
  }
...
export async function linkExistingDemoLogos({ sb, tenantId, bucket = DEMO_LOGO_BUCKET, log = console.log }) {
  ...
  if (missing > 0) {
    log(`[demo-logo] warning: ${missing} demo org(s) have no generated logo in storage yet — run the logo generation pass (see demo-seeds/README.md, "Images (avatars, logos & event headers)")`);
  }
