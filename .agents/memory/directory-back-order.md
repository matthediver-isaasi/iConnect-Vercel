...
Saved object-source keys must be treated as explicit positions, even if their historical order resembles an automatically appended sequence. Apply changing object list defaults only to keys absent from the selected saved order.
**Why:** Historical arrays carry no provenance distinguishing dragged positions from previously saved defaults; guessing would silently reset deliberate native/object interleaving.
**How to apply:** Do not bulk rewrite existing orders when changing metadata defaults. Keep directory-over-tenant precedence unchanged.
