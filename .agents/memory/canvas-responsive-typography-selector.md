...
**Rich-text data-fs spans:** breakpoint font sizes authored INSIDE Tiptap rich text (`data-fs-tablet`/`data-fs-mobile` spans) only apply if the rendering block emits `buildTiptapFontSizeResponsiveCss` in a `<style>` (registry.jsx helper, `[data-cb] [data-fs-*]` selector). Any block that renders sanitized rich text (Text, Hero Carousel slides...) must emit it for both editor (pinned bp) and public (@media) paths, placed AFTER any derived/tenant-style CSS so the author's explicit size wins at equal specificity — otherwise authored sizes are silently ignored and the tenant style value renders instead.
