...
- Booking counts are aggregate-only, `status='confirmed'` (mirrors checkinService), and only on events the member can see. Numeric aggregations (sum/avg/min/max) exist for whitelisted `numeric` catalog fields only and are refused on booking entities in both the validator and the booking executor — money columns must never become aggregable.
- Numeric aggregate results use `value: null, valueCount: 0` when no rows carry a value (never NaN or 0-as-average); grouped numeric truncation drops tail groups with NO "(other)" bucket (a merged bucket is meaningless for avg/min/max).
