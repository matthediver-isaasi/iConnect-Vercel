...
**How to apply:** Resolve the member first, then use the shared eligibility rule before exposing categories or writing `is_subscribed=true`. Keep server-side write checks even when the UI is filtered. Permit `is_subscribed=false` for a previously stored category after role access is removed.