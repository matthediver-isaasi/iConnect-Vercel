...
**Same trap in `validatePortalNav()`** (same file) for `branding_config.portalNav`: it rebuilds `out = {}` from a whitelist (background, userCard, pageBackground, the colour keys). Any new `portalNav.*` subfield must be added there or it silently vanishes on save and the portal renders the fallback. This is exactly why the Portal Page Background (pageBackground) feature's UI persisted but never rendered — the subfield was dropped until whitelisted.
