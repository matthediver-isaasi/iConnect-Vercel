...
- `link_relationship`: create one canonical `custom_object_relationship` edge for an active tenant relationship definition. `source_endpoint` and `target_endpoint` repeat the definition's exact endpoint kind and Custom Object UUID (when applicable). Each endpoint record ID comes either from a compatible submitted record field or from a compatible earlier record action output in that exact scope. In a repeatable action, a field source defaults to the current row but can explicitly use `source.scope: "form"` for a preceding top-level field (for example, one primary Organisation linked to each row Organisation); use `source.scope: "row"` to make that choice explicit. Forward references, cross-row references, reversed endpoints, inactive definitions, and unavailable records are rejected before an edge is accepted. These links are trusted persisted-form submission side effects, rather than browser-directed mutations of selected records; normal record create/update authorization is unchanged.
- `resolve_record_reference`: resolve one compatible record-backed picker to one canonical action output (`record_id` plus authoritative target kind and Custom Object ID where applicable). A saved existing ID is revalidated against the picker adapter, target metadata, tenant, and active record. A saved Not-listed sentinel instead uses `not_listed_operation` (`create` or `upsert`), the explicit `identity_mapping`, and `companion_mappings`. `reference_field_id` is the picker ID; the identity mapping's source is that picker's `not_listed_text`; companion mappings are separate same-scope field-to-target mappings. The picker, identity source, target fields, and companion fields are stable IDs; labels never imply compatibility. This operation is available at top level and per repeatable row, where all companion reads and outputs remain in that exact row.
- `resolve_record_references`: explicitly fan out a compatible multi-record picker. Every selected record ID is revalidated independently and the optional Not-listed item is created or upserted only through its explicit identity and companion mappings. Each item has a stable retry identity derived from the saved selection rather than its array position. The action emits a collection of the same canonical record outputs used by the single resolver. A later `link_relationship` action may consume that collection on one endpoint to create one canonical edge per resolved record; both endpoints cannot be collections, so accidental Cartesian products are rejected.
...
For repeatable rows, `_row_id` is the stable row retry identity. A multi-reference resolver adds a stable item identity (`record:<UUID>` or the Not-listed item identity), so reordering the picker array does not create new work. Runtime execution must create an idempotency/ledger entry keyed by the submission ID, action ID, row identity, and item identity (top-level actions omit only the row identity), and reuse the completed result on retry. It must not use row position or labels as an idempotency key. Relationship actions use that same ledger and also rely on the canonical active-edge uniqueness constraint, so a crash/retry cannot duplicate an edge. Runtime processors re-load both endpoint records in the submission tenant, re-resolve and authorize all saved metadata, and must not write Custom Object JSONB directly.

#### Extending record-reference resolution

Record-backed picker types opt in through the shared capability registry in `shared/formRecordReferenceResolver.js`; adding a Not-listed option to an ordinary choice field is not sufficient. An adapter must declare its authoritative target kind (and Custom Object ID when dynamic), cardinality, same-scope companion behaviour, and Not-listed identity source. The operation must explicitly match that cardinality: the original resolver accepts only single pickers and the fan-out resolver accepts only multi pickers. The adapter must also have server responsibilities that re-load and validate selected IDs, enforce tenant and active-definition boundaries, identify writable target fields, and create/upsert through the target's normal service.

The generic processor must reject an adapter when target metadata is missing or mismatched, selection cardinality is multiple, Not listed is disabled, identity mapping is absent, or companion fields escape the selected top-level/repeatable scope. Core and Custom Object adapters must emit the same canonical record output so later actions do not branch on how resolution occurred. New adapters must add shared pure compatibility tests plus server tests for stale and cross-tenant IDs, ambiguous identity matches, retries, and same-row isolation; they must not add field-label or tenant-form special cases to the processor.
