...
export default function Catalogue({ initialTab = "products" }) {
  ...
  const changeTab = (next) => { setTab(next); setSearch(""); };
  ...
  return <div className="catalogue-shell">
    ...
    <div className="mb-7 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
      <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-teal-700">Sales inventory</p><h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-slate-950">Product catalogue</h1><p className="mt-2 max-w-xl text-sm text-slate-600">Shape the items, pricing and packaged offers your team can quote with confidence.</p></div>
      <Button onClick={() => setDialog({ type: tab })} className="bg-teal-800 text-white hover:bg-teal-900"><Plus className="mr-2 h-4 w-4" />New {singular[tab]}</Button>
    </div>
    <Tabs value={tab} onValueChange={changeTab} className="mb-5">
      <TabsList className="h-auto w-full justify-start gap-1 rounded-xl border border-teal-950/10 bg-[#f2f4ef] p-1 sm:w-auto">
        {["categories", "products", "bundles"].map((key) => { const Icon = icon[key]; return <TabsTrigger key={key} value={key} className="gap-2 rounded-lg px-3 py-2 text-xs data-[state=active]:bg-teal-800 data-[state=active]:text-white"><Icon className="h-3.5 w-3.5" />{title[key]}</TabsTrigger>; })}
      </TabsList>
    </Tabs>
    <section className="overflow-hidden rounded-2xl border border-teal-950/10 bg-[#fffdf7] shadow-[0_16px_40px_-30px_rgba(8,73,70,.45)]">
      <div className="flex flex-col gap-3 border-b border-teal-950/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      {catalogue.isLoading ? <Skeleton /> : catalogue.isError ? <Failure onRetry={() => catalogue.refetch()} /> : rows.length === 0 ? <Empty type={tab} onCreate={() => setDialog({ type: tab })} /> : <CatalogueTable rows={rows} type={tab} categories={asList(categories.data)} products={asList(products.data)} onEdit={(row) => setDialog({ type: tab, row })} onArchive={(row) => { const action = row.isActive === false || row.archivedAt ? catalogue.restore : catalogue.archive; action.mutate(row.id, { onError: onMutationError }); }} onMove={move} />}
    </section>
...
function EditDialog({ config, categories, products, events, onClose, onSave, saving }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[90dvh] max-w-2xl overflow-y-auto border-teal-950/15 bg-[#fffdf7]"><form onSubmit={submit}><DialogHeader><DialogTitle className="font-serif text-2xl">{config.row ? "Edit" : "Add"} {singular[type]}</DialogTitle><DialogDescription>{type === "products" ? "Prices are stored as minor currency units; £125.00 is entered as 12500." : type === "bundles" ? "Set the included products and their display order." : "Organise how your sales catalogue is browsed."}</DialogDescription></DialogHeader><div className="mt-6 grid gap-4 sm:grid-cols-2">
  ...
  </div><DialogFooter className="mt-7"><Button type="button" variant="ghost" onClick={onClose}>Cancel</Button><Button disabled={saving} type="submit" className="bg-teal-800 text-white hover:bg-teal-900">{saving ? "Saving…" : "Save catalogue item"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
function ProductFields({ form, update, categories, events }) {
  ...
  return <>
    ...
    <Field label="Capacity metadata" wide><Textarea placeholder='e.g. {"venueZone":"Hall A"}' value={typeof form.capacityMetadata === "string" ? form.capacityMetadata : JSON.stringify(form.capacityMetadata || {})} onChange={(e) => update("capacityMetadata", e.target.value)} />{form.delegateCapacity && <p className="mt-1 text-xs text-teal-700">Delegate capacity is derived from the linked ticket: {form.delegateCapacity}</p>}</Field>
  </>;
}
