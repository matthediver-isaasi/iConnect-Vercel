...
import { Loader2, ArrowLeft, User, Pencil, Save, X, Building2, Mail, Smartphone, PhoneCall, Briefcase, Shield, CalendarDays, LogIn, Users, Globe, Key, Copy, Check, ClipboardList, FolderTree, Trophy, StickyNote, Calendar, Plus, Search, Trash2, MessageSquare, ChevronLeft, ChevronRight, ExternalLink } from "lucide-react";
...
export default function MemberDetail() {
  ...
  const { data: memberValues = [], isLoading: valuesLoading } = useQuery({
    queryKey: ['member-detail-preference-values', id],
    enabled: !!id,
    queryFn: async () => {
      try {
  ...
  const { data: communicationCategories = [], isLoading: communicationCategoriesLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`*, communication_category_role(role_id)`)
        .eq("is_active", true)
  ...
  // Sync formData with member prop
  useEffect(() => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data) => {
      return await base44.entities.Member.update(id, data);
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['member-detail-preference-values', id] });
    },
    onError: (error) => {
  ...
  const handleCommunicationToggle = async (categoryId, isSubscribed) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/members/${id}/communication-preferences/${categoryId}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
  ...
  const handleGenerateResetLink = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/members/${id}/generate-reset-link`, {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' }
      });
  ...
  const handleSaveOpeningBalances = async () => {
    ...
    try {
      await base44.entities.Member.update(id, {
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {memberCustomFields.filter(f => {
                      ...
                      const handleCustomFieldChange = (fieldId, newValue) => {
                        setCustomFieldValues(prev => ({ ...prev, [fieldId]: newValue }));
                      };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOpeningBalances(prev => ({ ...prev, eventsAttended: parseInt(e.target.value) || 0 }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOpeningBalances(prev => ({ ...prev, articlesPublished: parseInt(e.target.value) || 0 }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOpeningBalances(prev => ({ ...prev, jobsPosted: parseInt(e.target.value) || 0 }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOpeningBalances(prev => ({ ...prev, awards: parseInt(e.target.value) || 0 }))}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setOpeningBalances(prev => ({ ...prev, engagementAwards: parseInt(e.target.value) || 0 }))}
