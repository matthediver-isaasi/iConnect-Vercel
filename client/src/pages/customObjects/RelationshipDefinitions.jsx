...
import { CARDINALITIES, defaultDefinitionForm, definitionList, definitionPayload, ENTITY_KINDS, labelForSide } from "./relationshipHelpers";

const kindName = (kind) => ENTITY_KINDS.find(([value]) => value === kind)?.[1] || kind;

function DefinitionDialog({ objectId, definition, open, onOpenChange }) {
      ...
      <DialogContent className="max-h-[92dvh] max-w-3xl overflow-y-auto">
        ...
        <div className="space-y-6 py-2">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/80 p-4">
            ...
            <DirectionFields side="source" form={form} update={update} objects={objects} lockedKind="custom_object" immutable={Boolean(definition)} />
          </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50/80 p-4">
            ...
            <DirectionFields side="target" form={form} update={update} objects={objects} immutable={Boolean(definition)} />
          </div>
...
function DirectionFields({ side, form, update, objects, lockedKind, immutable }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-[150px_minmax(0,1fr)_minmax(0,1fr)]">
    ...
    <div>{form[kindKey] === "custom_object" ? <><Label>Custom object</Label><Select disabled={immutable} value={String(form[objectKey] || "")} onValueChange={(value) => update(objectKey, value)}><SelectTrigger><SelectValue placeholder="Choose active object" /></SelectTrigger><SelectContent>{objects.filter((item) => item.status === "active").map((item) => <SelectItem key={item.id} value={String(item.id)}>{item.plural_label}</SelectItem>)}</SelectContent></Select></> : <><Label>Endpoint</Label><div className="flex h-9 items-center rounded-md border bg-white px-3 text-sm text-slate-600">{kindName(form[kindKey])}</div></>}</div>
...
export function RelationshipDefinitions({ objectId, canManage }) {
