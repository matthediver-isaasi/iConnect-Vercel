...
import { LayoutDashboard, Plus, ShieldAlert, Sparkles, Users } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export default function ReportsDashboard() {
  ...
  const saveMutation = useMutation({
    mutationFn: async payload => {
      // Quick-resize path: payload carries the target widget id directly.
  ...
  const handleResize = widget => nextWidth => {
    // Optimistic resize so the grid updates instantly.
  ...
  const handleReorder = (scope, nextOrder) => {
    // Optimistic update so the UI feels snappy while the API confirms.
    qc.setQueryData(["/api/dashboard/widgets"], prev =>
      prev
        ? {
            ...prev,
  ...
  if (widgetsQuery.isError) {
    const status = widgetsQuery.error?.status;
    if (status === 403) {
      ...
      <DashboardShell>
        <Card>
          <CardContent className="py-10 text-center text-sm text-muted-foreground">
            Failed to load dashboard widgets. {widgetsQuery.error?.message}
          </CardContent>
        </Card>
      </DashboardShell>
    );
    ...
    <DashboardShell>
      <Card>
        <CardContent className="flex flex-col items-center gap-2 py-10 text-center">
          ...
          <p className="text-base font-medium">Dashboard access not enabled</p>
          <p className="max-w-md text-sm text-muted-foreground">
            Your role doesn’t currently include the dashboard. Ask an
            administrator to grant access in role permissions.
          </p>
        </CardContent>
      </Card>
    </DashboardShell>
