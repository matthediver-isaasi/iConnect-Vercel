...
test("monthly instalments load only after a linked history row is expanded", () => {
  ...
  assert.match(source, /source:\s*"personal"/);
...
test("monthly invoice actions use the authorized membership invoice endpoint", () => {
  assert.match(source, /onViewInvoice\?\.\(item\.paymentRef, item\.invoiceNumber, "instalment", item\.invoiceUrl\)/);
  assert.match(source, /onDownloadInvoice\?\.\(item\.paymentRef, item\.invoiceNumber, "instalment", item\.invoiceUrl\)/);
...
test("annual history invoice controls remain on the existing path", () => {
  assert.match(tabSource, /handleViewInvoice\(record\.id, invoiceNumber\)/);
  assert.match(tabSource, /handleDownloadInvoice\(record\.id, invoiceNumber\)/);
