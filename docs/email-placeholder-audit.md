...
| DD meeting request — resend | `api/dd-meeting-requests/resend.js` | ✅ (was ⚠️) | **FIXED** — generic helper pass covers `[[member.*]]` AND `[[organization.*]]` for the agent (member row extended to fetch `email` + `organization_id`; org row fetched and added to context, including underscore aliases). DD owner now resolved via `resolveDdOwnerForSubmission` from the meeting request's `form_submission_id` and applied to subject + body (`applyDdOwnerPlaceholders`); collapses to '' when no submission is linked. |
| DD meeting request — add alternative | `api/dd-meeting-requests/add-alternative.js` | ✅ (was ⚠️) | **FIXED** — same pattern as resend; agent's organization is fetched and merged into the placeholder context so `[[organization.*]]` resolves. DD owner now resolved from the request body's `formSubmissionId` and applied to subject + body. |
| Alternative-signer contract send (public nomination) | `api/public/alternative-signer/submit.js` | ✅ (was ⚠️) | **FIXED** — DD owner resolved via the contract instance's `form_submission_id`, falling back to `contract_settings.source_dd_form_id` (mirrors `send-original.js`); applied after the bespoke placeholder loop, collapses to '' when unresolvable. |
| Public meeting booking confirmation | `api/public/book/[slug].js` | ✅ (was ⚠️) | **FIXED** — when the booking links to a `dd_meeting_request` with a `form_submission_id`, DD owner is resolved and applied; with no DD linkage the replacement pass still runs with empty values so `{{dd_owner}}` / `{{dd_owner_email}}` / `[[dd_owner]]` never leak literally. |
