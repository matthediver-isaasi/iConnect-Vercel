#!/usr/bin/env node
/**
 * Make the Organisation Groups page assignable in /rolemanagement, default
 * disabled for non-admin roles.
 *
 * One-shot, idempotent data migration that:
 *   1. Locates the existing CRM module row in `role_access_item`
 *      (item_key=`crm`).
 *   2. Inserts the page row for `crm.organisation-groups`
 *      (item_type=page, parent_id=<crm module row id>) so the page
 *      becomes assignable in the role editor.
 *   3. Adds `crm.organisation-groups` to `excluded_features` for
 ...
 *
 * Usage:
 *   node scripts/seed-organisation-groups-role-access.mjs           # dry run
 *   node scripts/seed-organisation-groups-role-access.mjs --apply   # write changes
 */
...
const TAG = '[seed-organisation-groups-role-access]';
...
const MODULE_ICON = 'Database';

const PAGE_KEY = 'crm.organisation-groups';
const PAGE_LABEL = 'Organisation Groups';
...
async function ensureModuleRow() {
  ...
  if (existing) {
    console.log(`${TAG} CRM module row already present (id=${existing.id}).`);
    return existing;
  }
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert CRM module row.`);
    return { id: null };
  }
  ...
  console.log(`${TAG} Inserted CRM module row (id=${created.id}).`);
  return created;
}
...
async function ensurePageRow(moduleId) {
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert Organisation Groups page row.`);
    return null;
  }
  ...
  const { data: created, error: insErr } = await supabase
    ...
    .single();
  if (insErr) throw insErr;
  console.log(`${TAG} Inserted Organisation Groups page row (id=${created.id}).`);
  return created;
}
