...
// tenant-owned): Hero H1 - White, Subsection H3 White, Section H2,
// Subsection H3, Body.
const TYPO = {
...
// colour band of the existing award/application pages. The UKRG theme mirrors
// the existing hand-built /about-ukrg page (red accent, light-red band, blue
// hero gradient, tenant:ukrg button style) so the new UKRG pages sit
// consistently alongside it. Selected per-page via spec.theme.
const THEMES = {
  bnms: {
    accent: '#fa8300',
    bandBg: '#F4F7FF',
  ...
  // MRT (UK Molecular Radiotherapy Consortium) — deep-crimson accent, warm
  // cream band, orange->transparent hero gradient, tenant:mrt buttons. Mirrors
  // the existing hand-built /about-mrt page. logoUrl adds the consortium logo
  // top-right on the opening hero (see makeLogo + buildDesign).
  mrt: {
    accent: '#940c1f',
    bandBg: '#fffbf5',
    dividerColor: '#940c1f',
...
// Mutable active theme — set at the top of buildDesign() from spec.theme. The
// block factories read THEME at call time (buildDesign runs synchronously start
// to finish per page, so there is no cross-page bleed).
...
function makeHero({ headline, subheadline, ctaLabel, bgImageUrl }, geom) {
  return wrap({
    ...
    content: {
      ctas: [
        {
          ...
          labelTypographyStyleId: TYPO.heroSub,
        },
      ],
      bgType: 'image',
      ...
      headlineTypographyStyleId: TYPO.heroHeadline,
      subheadlineTypographyStyleId: TYPO.heroSub,
    },
  });
}
...
// chip so the (JPEG) consortium logo reads cleanly over the dark-washed hero
// image; higher zIndex + emitted after the hero so it layers above it.
function makeLogo(src, geom) {
  return wrap({
    ...
    locked: false,
    content: {
      alt: 'UK Molecular Radiotherapy Consortium logo',
      src,
      href: '',
      iconSize: 50,
      fullBleed: false,
...
// border + shadow, orange accent icon, tenant card-heading typography). CTA is
// optional; when present it renders the tenant primary button style, link "#".
function makeCard({ icon, heading, body, cta }, geom) {
  return wrap({
    ...
    content: {
      ...
      headingTypographyStyleId: TYPO.cardHeading,
    },
  });
}
...
function makeButton(label, geom) {
  return wrap({
    ...
    content: {
      ...
      ariaLabel: '',
      sizeClass: 'lg',
      typographyStyleId: TYPO.heroSub,
    },
  });
}

// Dashed "note" box — used for deferred/interactive surfaces (searchable
// member directory, video embed, searchable recipients table) that will be
// wired up later. Renders muted secondary text inside a dashed accent frame.
...
const HERO_H = 600;
const CLOSING_HERO_H = 420;

function buildDesign(spec) {
  THEME = THEMES[spec.theme] || THEMES.bnms;
  const topBlocks = [];
  const sectionBlocks = [];
  let y = 0;
