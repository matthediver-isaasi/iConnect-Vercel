#!/usr/bin/env node
/**
 * Task #3118 Phase 3: Make the Monthly Finance Report assignable in
 ...
 *      (item_key=`commerce`).
 *   2. Inserts the page row for `commerce.monthly-finance-report`
 *      (item_type=page, parent_id=<commerce module row id>) so the page
 *      becomes assignable in the role editor.
 *   3. Adds `commerce.monthly-finance-report` to `excluded_features` for
 ...
 *
 * Usage:
 *   node scripts/seed-monthly-finance-report-role-access.mjs           # dry run
 *   node scripts/seed-monthly-finance-report-role-access.mjs --apply   # write changes
 */

import { createClient } from '@supabase/supabase-js';

const APPLY = process.argv.includes('--apply');
const TAG = '[seed-monthly-finance-report]';
...
const PAGE_KEY = 'commerce.monthly-finance-report';
const PAGE_LABEL = 'Monthly Finance Report';
...
async function ensurePageRow(moduleId) {
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert Monthly Finance Report page row.`);
    return null;
  }
  ...
  const { data: created, error: insErr } = await supabase
    ...
    .single();
  if (insErr) throw insErr;
  console.log(`${TAG} Inserted Monthly Finance Report page row (id=${created.id}).`);
  return created;
}
