/**
 * Apply the Task #1550 vacancy positions + awarding migration to the
 * destination Supabase over the IPv4 pooler (DEST_DATABASE_URL). Idempotent;
 * safe to re-run.
 *
 * Adds:
 *   - vacancy.positions_available (integer, default 1)
 *   - vacancy_award table (tenant-scoped + group-scoped award records)
 *
 * Usage:
 *   node scripts/apply-vacancy-positions-awards.mjs
 */
...
const MIGRATIONS = [
  'supabase/migrations/20260618_vacancy_positions_awards.sql',
];
