/**
 * Apply the booking replacement title migration (event + complex_event).
 *
 * Usage:
 *   node scripts/apply-booking-replacement-title-migration.mjs
 */
...
const MIGRATIONS = [
  'supabase/migrations/20260806_add_booking_replacement_title.sql',
];
...
async function run() {
  ...
  try {
    ...
    const check = await client.query(
      "SELECT table_name, column_name FROM information_schema.columns WHERE column_name = 'booking_replacement_title' AND table_name IN ('event', 'complex_event')"
    );
