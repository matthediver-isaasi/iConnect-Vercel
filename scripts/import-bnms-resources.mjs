/**
 * Task #1087 — One-off import: load BNMS resources CSV into the `resource`
 * table for tenant BNMS (ff2df806-b321-4254-b651-3af11fccf1db).
 *
 * Usage:
 *   node scripts/import-bnms-resources.mjs --dry-run
 *   node scripts/import-bnms-resources.mjs
 *   node scripts/import-bnms-resources.mjs --tenant=<uuid> --csv=<path>
 *   node scripts/import-bnms-resources.mjs --limit=5
 *   node scripts/import-bnms-resources.mjs --create-missing-categories
 *
...
const CSV_PATH = args.csv || './attached_assets/BNMSResourcesClean_1779888089327.csv';
...
// Resource Type column (H) header values that map to "Resource Type" category.
const RESOURCE_TYPE_CATEGORY = 'Resource Type';
const FOCUS_AREA_CATEGORY = 'Focus Area';

// Columns to ignore (A,B) and the special "ResourceType" col (H).
const META_COLUMNS = new Set([
  'Page URL', 'Menu Item', 'Resource URL', 'Title', 'Brief Description',
  'Date', 'Member Only', 'Resource Type',
]);
...
async function main() {
  ...
  // Resource Type values used in column H (every row uses "Guidelines" per task).
