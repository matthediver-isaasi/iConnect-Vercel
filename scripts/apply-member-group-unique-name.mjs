/**
 * Apply the Task #1245 member group unique-name migration.
 *
 * Merges any pre-existing duplicate group names (case-insensitive, per tenant)
 * and adds a tenant-scoped unique index on member_group(tenant_id, lower(name)).
 * Idempotent.
 *
 * Usage:
 *   node scripts/apply-member-group-unique-name.mjs
 */
...
const MIGRATIONS = [
  'supabase/migrations/20260603_member_group_unique_name.sql',
];
