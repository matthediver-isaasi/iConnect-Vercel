/**
 * BNMS Spring Meeting 2026 resource reconciliation.
 *
 * This module is deliberately read-only.  It parses the pinned workbook,
 * performs conservative identity matching, and builds an auditable plan for
 * the DEST tenant.  The application runner owns all writes.
 */
import XLSX from 'xlsx';
import { createHash } from 'node:crypto';
import { parseDate } from './bnms-youtube-categorisation.mjs';
...
export const INPUT = 'attached_assets/Resources_-_categorising-tagging_2026_PRESENTATIONS_AND_POSTE_1789489599381.xlsx';
export const SOURCE_CHECKSUM = 'db61ce39f47baec8a8bb48bc2c8a0a40081163193e139b816e0e6300d3b5c1e0';
export const SOURCE_ROWS = 308;
export const DOWNLOAD_RESOURCE_TYPE = 'download';
...
export function link(raw) {
  ...
  try {
    ...
    const folder = url.pathname.match(
      /^\/drive\/(?:u\/\d+\/)?folders\/([A-Za-z0-9_-]+)\/?$/,
    );
    if (folder && ['drive.google.com', 'docs.google.com'].includes(url.hostname)) {
      return { valid: true, identity: `folder:${folder[1]}`, kind: 'shared_folder' };
    }
    const id = driveId(value);
    if (id) return { valid: true, identity: `drive:${id}`, kind: 'drive_file' };
...
function readCellLinksAndFormulas(sheet, rowIndex, headers) {
  const hyperlinks = [];
  const formulas = [];
...
// Ordered, exact-count checked reads.  The stable sort and actual page length
// are both intentional: neither assumes a PostgREST 1000-row response cap.
export async function readAll(client, table, columns) {
  ...
  do {
    ...
    if (!Array.isArray(data) || !Number.isInteger(count)
      ...
      throw new Error(`${table}: inconsistent pagination count`);
    }
    total = count;
    pages.push(data.length);
    if (!data.length && rows.length < total) throw new Error(`${table}: premature empty page`);
    for (const row of data) {
      if (row.tenant_id !== TENANT_ID) throw new Error(`${table}: tenant isolation failure`);
      if (rows.length && String(row.id) <= String(rows.at(-1).id)) {
        throw new Error(`${table}: repeated/unordered IDs`);
      }
      rows.push(row);
    }
...
export function buildReport(workbook, resources, categories) {
  ...
  for (const name of ['Collection', 'Resource Type', 'Focus Area']) {
    if (!taxonomy[name]) throw new Error(`Missing, ambiguous or invalid taxonomy: ${name}`);
  }
  ...
  const rows = workbook.rows.map((workbookRow) => {
    ...
    for (const hyperlink of workbookRow.hyperlinks) {
      ...
      const literal = decodeHref(hyperlink.literal);
      const target = decodeHref(hyperlink.target);
      if (literal !== target) {
        const literalLink = link(literal);
        const targetLink = link(target);
        if (literalLink.identity && literalLink.identity === targetLink.identity) {
    ...
    for (const [categoryName, values] of Object.entries(selected)) {
      ...
      for (const value of values) {
        if (!taxonomy[categoryName].subcategories.includes(value)) {
    ...
    const titleCandidates = candidates.length === 0
      ? resources
        .filter((resource) => trim(resource.title).toLowerCase() === trim(source.Title).toLowerCase())
    ...
    Object.assign(proposed, {
      ...
      // An identity match retains its stored URL variant.  The literal source
      // URL is used only when inserting and is retained in the audit either way.
      target_url: before?.target_url ?? sourceUrl,
      description: trim(source['Brief Description']) || before?.description || '',
      // Blank source dates never erase a stored date.
      release_date: date.value || before?.release_date || null,
      // The user explicitly approved the Member Only Yes override.  A No row
      // is still conservative for existing restricted records.
    ...
    const displayTypeChange = Boolean(before && Object.hasOwn(patch, 'resource_type'));
  ...
  const coreChangeFields = Object.fromEntries(CORE_FIELDS.map((field) => [
    field,
    count((row) => row.coreChanges.includes(field)),
  ]));
  const summary = {
    ...
    exactMatchRows: count((row) => row.matchMethod === 'exact_url'),
    providerIdentityMatchRows: count((row) => row.matchMethod === 'provider_identity'),
    identityMatchRows: count((row) => row.matchMethod === 'provider_identity'),
    unmatchedRows: count((row) => row.matchMethod === 'none'),
    multipleDestinationRows: count((row) => row.issues.includes('multiple_destination_matches')),
    titleOnlyRows: count((row) => row.issues.includes('title_only_candidate_requires_review')),
    coreChangeRows: count((row) => row.coreChanges.length > 0),
    coreChangeFields,
    accessChangeRows: count((row) => row.accessChange),
    nonBlockedAccessChangeRows: count(
      (row) => row.status !== 'blocked' && row.accessChange,
    ),
    classificationChangeRows: count((row) => row.before && row.patch.subcategories),
    displayTypeChangeRows: count((row) => row.displayTypeChange),
    yearOnlyRows: count((row) => row.date.kind === 'year_only_requires_approval'),
    invalidDateRows: count((row) => row.issues.includes('invalid_date')),
    missingTaxonomyRows: count((row) => row.issues.some(
    ...
    hyperlinkRows: count((row) => row.hyperlinks.length > 0),
    hyperlinkConflicts: count((row) => row.issues.some(
      (issue) => issue.startsWith('hyperlink_conflict:'),
    )),
    formulaRows: count((row) => row.formulas.length > 0),
    databaseWrites: 0,
  };
  return {
