#!/usr/bin/env node
/**
 * Task #3223: Make the Group Classification Activity Report assignable in
 * /rolemanagement, default disabled for non-admin roles.
 *
 * One-shot, idempotent data migration that:
 *   1. Locates the existing Membership module row in `role_access_item`
 *      (item_key=`membership`).
 *   2. Inserts the page row for `membership.member-group-classification-report`
 *      (item_type=page, parent_id=<membership module row id>) so the page
 *      becomes assignable in the role editor.
 *   3. Adds `membership.member-group-classification-report` to `excluded_features` for
 ...
 *
 * Usage:
 *   node scripts/seed-member-group-classification-report-role-access.mjs           # dry run
 *   node scripts/seed-member-group-classification-report-role-access.mjs --apply   # write changes
 */

import { createClient } from '@supabase/supabase-js';

const APPLY = process.argv.includes('--apply');
const TAG = '[seed-member-group-classification-report]';
...
const MODULE_KEY = 'membership';
const MODULE_LABEL = 'Membership';
const MODULE_ICON = 'Users';

const PAGE_KEY = 'membership.member-group-classification-report';
const PAGE_LABEL = 'Group Classification Activity Report';
...
async function ensureModuleRow() {
  ...
  if (existing) {
    console.log(`${TAG} Membership module row already present (id=${existing.id}).`);
    return existing;
  }
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert Membership module row.`);
    return { id: null };
  }
  ...
  console.log(`${TAG} Inserted Membership module row (id=${created.id}).`);
  return created;
}
...
async function ensurePageRow(moduleId) {
  ...
  // Slot after the existing invite-report page within the module.
  let displayOrder = 0;
  if (moduleId) {
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert Group Classification Activity Report page row.`);
    return null;
  }
  ...
  const { data: created, error: insErr } = await supabase
    ...
    .single();
  if (insErr) throw insErr;
  console.log(`${TAG} Inserted Group Classification Activity Report page row (id=${created.id}).`);
  return created;
}
