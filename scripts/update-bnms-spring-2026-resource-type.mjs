/**
 * Task #2146 — One-off update: set `resource_type = 'download'` on the BNMS
 * "Spring Meeting 2026" resources that Task #2143 imported as 'external_link'.
 *
 * These 308 resources are all Google Drive PDF links, so their CTA button
 * should read "Download" (Download icon) instead of "Visit Site" — see
 * client/src/components/resources/ResourceCard.jsx. Click behaviour is
 * unchanged; the target_url still opens in a new tab.
 *
 * Target rows: `resource` rows for tenant BNMS
 * (ff2df806-b321-4254-b651-3af11fccf1db) whose `subcategories` array contains
 * the Collection tag "Spring Meeting 2026" AND whose current `resource_type`
 * is 'external_link'. Expected match count: 308.
 *
 ...
 * scoped to tenant + resource_type='external_link').
 *
 * Usage:
 *   node scripts/update-bnms-spring-2026-resource-type.mjs --dry-run
 *   node scripts/update-bnms-spring-2026-resource-type.mjs
 *   node scripts/update-bnms-spring-2026-resource-type.mjs --tenant=<uuid>
 *   node scripts/update-bnms-spring-2026-resource-type.mjs --limit=5
 *   node scripts/update-bnms-spring-2026-resource-type.mjs --xlsx=<path>
 *
 * Idempotent: only rows still at 'external_link' are updated; re-runs match
 * fewer/zero rows once flipped to 'download'.
 */
...
const OLD_TYPE = 'external_link';
const NEW_TYPE = 'download';
const EXPECTED = 308;
...
async function loadAllResources() {
  ...
  for (let from = 0; ; from += PAGE) {
    const { data: page, error } = await supabase
      .from('resource')
      .select('id, title, target_url, resource_type, subcategories')
...
async function main() {
  ...
  console.log(`Matching: resource_type='${OLD_TYPE}' -> '${NEW_TYPE}'`);
  ...
  if (XLSX_PATH) {
    ...
    matched = resources.filter(
      (r) => r.resource_type === OLD_TYPE && r.target_url && urls.has(r.target_url),
    );
  } else {
    matched = resources.filter(
      (r) =>
        r.resource_type === OLD_TYPE &&
        Array.isArray(r.subcategories) &&
        r.subcategories.includes(COLLECTION_TAG),
    );
  }

  console.log(`\nMatched ${matched.length} rows (expected ${EXPECTED}).`);
  if (matched.length !== EXPECTED) {
    console.log(
      `  NOTE: match count differs from expected ${EXPECTED}. Review before writing` +
