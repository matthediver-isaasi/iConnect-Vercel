#!/usr/bin/env node
/**
 * Make the Event Budget Report assignable in /rolemanagement, default
 * disabled for non-admin roles.
 *
 * One-shot, idempotent data migration that:
 *   1. Locates the existing Events module row in `role_access_item`
 *      (item_key=`events`).
 *   2. Inserts the page row for `events.event-budget-report`
 *      (item_type=page, parent_id=<events module row id>) so the page
 *      becomes assignable in the role editor.
 *   3. Adds `events.event-budget-report` to `excluded_features` for
 ...
 *
 * Usage:
 *   node scripts/seed-event-budget-report-role-access.mjs           # dry run
 *   node scripts/seed-event-budget-report-role-access.mjs --apply   # write changes
 */
...
const TAG = '[seed-event-budget-report]';
...
const MODULE_LABEL = 'Events';
const MODULE_ICON = 'Calendar';

const PAGE_KEY = 'events.event-budget-report';
const PAGE_LABEL = 'Event Budget Report';

// Admin-level roles are determined by the PERMISSION model, not role names:
// server-side hasAdminAccess treats a role as admin when it retains access to
// admin.role-management (see api/_lib/tenantContext.js). Mirror that here.
function isAdminRole(excludedFeatures) {
  const list = Array.isArray(excludedFeatures) ? excludedFeatures : [];
  return !isResourceExcluded(list, 'admin.role-management');
}
...
async function ensureModuleRow() {
  ...
  if (existing) {
    console.log(`${TAG} Events module row already present (id=${existing.id}).`);
    return existing;
  }
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert Events module row.`);
    return { id: null };
  }
  ...
  console.log(`${TAG} Inserted Events module row (id=${created.id}).`);
  return created;
}
...
async function ensurePageRow(moduleId) {
  ...
  if (!APPLY) {
    console.log(`${TAG} DRY RUN: would insert Event Budget Report page row.`);
    return null;
  }
  ...
  const { data: created, error: insErr } = await supabase
    ...
    .single();
  if (insErr) throw insErr;
  console.log(`${TAG} Inserted Event Budget Report page row (id=${created.id}).`);
  return created;
}
